"""Constant-velocity linear Kalman filter plugin for KalmanLab.

This filter estimates 2D position and velocity:

    x = [px, py, vx, vy]^T

GNSS provides position measurements. IMU acceleration is not part of the
state vector. Instead, the latest IMU acceleration is used as an optional
control input during prediction:

    px_k = px + vx*dt + 0.5*ax*dt^2
    py_k = py + vy*dt + 0.5*ay*dt^2
    vx_k = vx + ax*dt
    vy_k = vy + ay*dt

So the model is still CV in state definition, but IMU helps propagate the
state between GNSS updates.
"""
...
TUNE = {
    ...
    "initial_velocity_stddev_mps": 4.0,

    # Yaw estimation behavior.
    "yaw_from_velocity_min_speed_mps": 0.35,

    # Prediction dt guards.
...
class _CVFilterCore:
    """Linear 2D constant-velocity Kalman filter."""

    def __init__(self, tune: dict[str, float]) -> None:
        self._process_accel_var = float(tune["process_accel_stddev_mps2"]) ** 2
    ...
    def predict(
        ...
        ax = float(acceleration_xy[0])
        ay = float(acceleration_xy[1])
    ...
    def snapshot(self) -> Optional[_CVStateSnapshot]:
        if not self.initialized or self._timestamp is None:
            return None

        return _CVStateSnapshot(
    ...
    @staticmethod
    def _control_matrix(dt: float) -> np.ndarray:
        half_dt2 = 0.5 * dt * dt
        return np.array(
            [
                [half_dt2, 0.0],
    ...
    def _process_noise(self, dt: float) -> np.ndarray:
        ...
        q_axis = self._process_accel_var * np.array(
            [
                [dt4 / 4.0, dt3 / 2.0],
                [dt3 / 2.0, dt2],
            ],
...
class Filter:
    ...
    def process_imu(self, imu: "ImuMeasurement") -> Optional[VehicleState]:
        ...
        self._latest_acceleration_xy = self._imu_acceleration_to_world_xy(imu, yaw_deg)
    ...
    def _yaw_deg_from_compass(compass_rad: float) -> Optional[float]:
        ...
        # Same convention as the existing CA-KF:
        # CARLA compass is converted to a vehicle/world yaw-like angle.
